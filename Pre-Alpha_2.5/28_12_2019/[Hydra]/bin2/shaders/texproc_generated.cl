...
float4 prtex3_main(const SurfaceInfo* sHit, float3 colorHit, sampler2D texHit, float3 colorMiss, sampler2D texMiss, float falloffPower, int useDownAO,  __global const float4* restrict in_texStorage1, __global const EngineGlobals* restrict in_globals, const float3 hr_viewVectorHack)
{
  const float2 texCoord = readAttr_TexCoord0(sHit);
  const float aoUp      = readAttr_AO(sHit);
  const float aoDown    = readAttr_AO1(sHit);
                          ...
                          int iNumElements)
{
  ...
  if (materialGetFlags(pHitMaterial) & PLAIN_MATERIAL_HAVE_PROC_TEXTURES)
  {
    ...
    int counter = 0; 
    if(materialHeadHaveTargetProcTex(pHitMaterial,3) && counter < MAXPROCTEX)
    {
      __global const float* stack = fdata + findArgDataOffsetInTable(3, table);
      ptl.fdata4[counter] = to_float3(prtex3_main(sHit, make_float3(stack[0], stack[1], stack[2]), as_int(stack[3]), make_float3(stack[4], stack[5], stack[6]), as_int(stack[7]), stack[8], as_int(stack[9]), in_texStorage1, in_globals, hr_viewVectorHack));
      ptl.id_f4 [counter] = 3;
      counter++;
    }

    ptl.currMaxProcTex = counter;
    // BREAK SHADER CACHE AT: 2019-12-27.20:07:53
